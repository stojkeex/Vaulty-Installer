...
export default function Login() {
  ...
  const getRandomShootingStar = () => {
    const animations = [
      'shooting-star-diagonal-1',
      'shooting-star-diagonal-2',
      'shooting-star-horizontal',
      'shooting-star-vertical',
      'shooting-star-diagonal-3'
    ];
    const randomAnimation = animations[Math.floor(Math.random() * animations.length)];
    const randomDuration = 2.5 + Math.random() * 2; // 2.5s to 4.5s
    const randomYOffset = Math.random() * 100 - 50; // -50% to 50%

    return { randomAnimation, randomDuration, randomYOffset };
  };

  const { randomAnimation, randomDuration, randomYOffset } = getRandomShootingStar();
    ...
    >
      ...
      <div key={shootingStarKey} className="absolute inset-0 z-0 pointer-events-none">
        <div 
          ...
          style={{
            left: '0',
            top: `calc(50% + ${randomYOffset}%)`,
            boxShadow: '0 0 20px 4px rgba(255,255,255,0.8), 0 0 40px 8px rgba(255,255,255,0.4)',
            filter: 'drop-shadow(0 0 15px rgba(255,255,255,0.6))',
            animation: `${randomAnimation} ${randomDuration}s ease-in-out forwards`
          }} 
        />
      </div>
