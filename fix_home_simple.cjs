...
// 1. Remove Goals, Vaulty Coin, Invite Friends section
...
if (reachGoalIndex !== -1) {
  ...
  if (gridStart !== -1) {
    const newsSectionMatch = content.match(/<div className="relative overflow-hidden rounded-\[32px\] border border-white\/10 bg-\[linear-gradient\(180deg,rgba\(255,255,255,0\.08\),rgba\(255,255,255,0\.03\)\)\] p-6/);
    if (newsSectionMatch) {
      content = content.substring(0, gridStart) + '<div className="mt-8">\n          <div className="space-y-8">\n            {/* Finance News Card */}\n            ' + content.substring(newsSectionMatch.index);
    }
  }
}

// 2. Remove Academy and Investment Simulator from left column
...
// 3. Merge Financial Health and Freedom Map into one
...
if (topRowMatch) {
  ...
  if (netWorthMatch && freedomMapMatch) {
    ...
    freedomMapInner = freedomMapInner.replace('space-y-4 flex-1 flex flex-col justify-center', 'space-y-3 flex flex-col pt-2');
    freedomMapInner = freedomMapInner.replace('<div className="flex items-center justify-between mb-6 shrink-0">', '<div className="flex items-center justify-between mb-4 shrink-0 hidden">'); // Hide the inner header
...
console.log("Safe refactoring applied.");
